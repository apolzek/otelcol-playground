...
echo "==> Rodando telemetrygen: traces, duration=${DURATION}, workers=${WORKERS}, 20 attrs"
docker compose run --rm telemetrygen >/dev/null
...
{
  echo "===================================================="
  echo "scenario-01 — gRPC l1 -> gRPC l2 (sem batch, sem nada)"
  ...
  echo
  echo "  Bytes l1 -> l2 (gRPC) ≈ otel-l2.rx = $(human $L2_RX)  (= ${L2_RX} B)"
  echo
