...
Cinco abordagens distintas para expor um **OpenTelemetry Collector v0.154.0** publicamente na internet rejeitando fontes desconhecidas. Cada pasta é independente e roda com `docker compose up`.

| # | Pasta | Mecanismo | Identifica cliente? | Custo por req | Rotação | Quando usar |
|---|-------|-----------|:-------------------:|---------------|---------|-------------|
| 1 | [`01-bearer-token`](01-bearer-token/)   | Bearer estático + API Go               | parcial (token→tenant na API) | <1µs   | API + SIGHUP   | volume alto, integração simples |
...
```mermaid
quadrantChart
  title Simplicidade vs força de identificação
  x-axis "Simples" --> "Complexo"
  y-axis "Identificação fraca" --> "Identificação forte"
  "01 bearer": [0.12, 0.30]
  "04 basic-auth": [0.30, 0.55]
  "02 oidc-jwt": [0.65, 0.80]
  "03 mtls": [0.80, 0.90]
  "05 hmac-proxy": [0.92, 0.70]
```
...
- Versão fixa do collector: `otel/opentelemetry-collector-contrib:0.154.0` (não usar `latest`)

## Convenções

- Todos os Go services rodam como UID `65532` (nonroot do distroless), bind em portas não privilegiadas.
- Nas abordagens 1 e 4 o collector também roda como `65532` para ler o arquivo de credenciais (`0600`) do volume compartilhado.
- Reload em 1 e 4: a API regrava o arquivo e envia `SIGHUP` — o collector relê via restart (sem reload gráfico nativo).
